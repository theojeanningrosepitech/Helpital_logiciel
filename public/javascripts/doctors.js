...
function search() {
    apiGet('/doctors?list=true&search=' + document.getElementById('searchInput').value, function(response) {
...
function initDoctors() {
    ...
    websocket.addEventListener('user', 'new', (message) => {
        ...
        if (message.data.user_role == '3') {
            fetch('/doctors/fragment?id=' + message.identifiers.id).then(function(response) {
